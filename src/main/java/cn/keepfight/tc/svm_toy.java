...
import cn.keepfight.tc.libsvm.svm_problem;
...
public class svm_toy extends Applet {
	...
	void button_run_clicked(String args)
	{
		...
		cn.keepfight.tc.libsvm.svm_parameter param = new cn.keepfight.tc.libsvm.svm_parameter();

		// default values
		param.svm_type = cn.keepfight.tc.libsvm.svm_parameter.C_SVC;
		param.kernel_type = cn.keepfight.tc.libsvm.svm_parameter.RBF;
		param.degree = 3;
		param.gamma = 0;
		param.coef0 = 0;
		param.nu = 0.5;
		...
		// build problem
		cn.keepfight.tc.libsvm.svm_problem prob = new svm_problem();
		prob.l = point_list.size();
		prob.y = new double[prob.l];

		if(param.kernel_type == cn.keepfight.tc.libsvm.svm_parameter.PRECOMPUTED)
		{
		}
		else if(param.svm_type == cn.keepfight.tc.libsvm.svm_parameter.EPSILON_SVR ||
			param.svm_type == cn.keepfight.tc.libsvm.svm_parameter.NU_SVR)
		{
			if(param.gamma == 0) param.gamma = 1;
			prob.x = new cn.keepfight.tc.libsvm.svm_node[prob.l][1];
			for(int i=0;i<prob.l;i++)
			{
				point p = point_list.elementAt(i);
				prob.x[i][0] = new cn.keepfight.tc.libsvm.svm_node();
				prob.x[i][0].index = 1;
			...
			cn.keepfight.tc.libsvm.svm_model model = cn.keepfight.tc.libsvm.svm.svm_train(prob, param);
			cn.keepfight.tc.libsvm.svm_node[] x = new cn.keepfight.tc.libsvm.svm_node[1];
			x[0] = new cn.keepfight.tc.libsvm.svm_node();
			x[0].index = 1;
			int[] j = new int[XLEN];
			...
			for (int i = 0; i < XLEN; i++)
			{
				...
				j[i] = (int)(YLEN* cn.keepfight.tc.libsvm.svm.svm_predict(model, x));
			}
			...
			for(int i=1;i<XLEN;i++)
			{
				...
				if(param.svm_type == cn.keepfight.tc.libsvm.svm_parameter.EPSILON_SVR)
				{
		...
		else
		{
			...
			prob.x = new cn.keepfight.tc.libsvm.svm_node[prob.l][2];
			for(int i=0;i<prob.l;i++)
			{
				point p = point_list.elementAt(i);
				prob.x[i][0] = new cn.keepfight.tc.libsvm.svm_node();
				prob.x[i][0].index = 1;
				prob.x[i][0].value = p.x;
				prob.x[i][1] = new cn.keepfight.tc.libsvm.svm_node();
				prob.x[i][1].index = 2;
			...
			// build model & classify
			cn.keepfight.tc.libsvm.svm_model model = cn.keepfight.tc.libsvm.svm.svm_train(prob, param);
			cn.keepfight.tc.libsvm.svm_node[] x = new cn.keepfight.tc.libsvm.svm_node[2];
			x[0] = new cn.keepfight.tc.libsvm.svm_node();
			x[1] = new cn.keepfight.tc.libsvm.svm_node();
			x[0].index = 1;
			x[1].index = 2;
			...
			for (int i = 0; i < XLEN; i++)
				for (int j = 0; j < YLEN ; j++) {
					...
					double d = cn.keepfight.tc.libsvm.svm.svm_predict(model, x);
					if (param.svm_type == cn.keepfight.tc.libsvm.svm_parameter.ONE_CLASS && d<0) d=2;
	...
	void button_save_clicked(String args)
	{
		...
		try {
			...
			int svm_type = cn.keepfight.tc.libsvm.svm_parameter.C_SVC;
			...
			int n = point_list.size();
			if(svm_type == cn.keepfight.tc.libsvm.svm_parameter.EPSILON_SVR || svm_type == cn.keepfight.tc.libsvm.svm_parameter.NU_SVR)
			{
				for(int i=0;i<n;i++)
				{
					point p = point_list.elementAt(i);
	...
	{
		new AppletFrame("cn.keepfight.tc.svm_toy",new svm_toy(),500,500+50);
	}
}
